...
- **Fixed:** a `ForPath` mapping targeting a leaf property that legitimately resolves to `null` (e.g. an optional `string`) either silently discarded that `null` in favor of a freshly-constructed instance, or — for a leaf type with no public parameterless constructor reachable via reflection (`string` itself, most commonly) — threw `InvalidOperationException` outright. `Mapper.ApplyNestedMapping` walked one segment too far: after applying the mapped value to the path's actual leaf segment, it unconditionally tried to descend into it as if it were another intermediate object. Only intermediate segments need an instance to walk through, so the leaf segment is no longer touched after its value is assigned. See `MapperTests.Map_UsesForPath_WhenLeafValueIsNull`.
- **Added:** same-name nested collection destination *properties* now also accept an array (`T[]`) destination — the nested-property equivalent of the array support `Map<TDestination>(object)` already had since 4.4.0. Previously, a `List<TSource>` source property mapping into a `TDest[]` destination property (with a registered map for the item types) was silently left unmapped, the same as any other unregistered type mismatch; it is now populated via a new `MappingPlanCompiler.BuildNestedArrayAssignment`/`Mapper.TryGetListToArrayItemTypes` path, mirroring the existing `List<T>`/interface-destination path (`BuildNestedListAssignment`/`TryGetListItemTypes`) rather than widening it, since `List<T>` is not reference-assignable to `T[]`. Purely additive: no previously-mapped output changes, since this only fills in a property that was previously always left at its default. See `MapperTests.Map_AutoMapsNestedArrayProperty_WhenItemTypesHaveRegisteredMap`.
- **Documentation fixes** (no code-behavior changes): several claims left over from before the 4.4.0 mapping-engine redesign were stale and have been corrected:
  - The guide and FAQ described destination-instance construction as `Activator.CreateInstance`; since the 4.4.0 compiled-plan redesign, top-level destination construction is a compiled constructor call (`Expression.New`), which is also why value-type destinations without an explicit parameterless constructor work. Wording updated to match `CLAUDE.md`'s own architecture description.
  - The home page's "Quick answers" FAQ entry comparing AutoMapperLite to AutoMapper/Mapster still said AutoMapperLite was "slower than both... especially collections" — true before 4.4.0's collection-loop fix, false since: AutoMapperLite has beaten AutoMapper on collection mapping at every size from 1,000 items upward since 4.4.0 (see `docs/performance.html`). Updated to state the current, measured comparison.
  - The FAQ's visible "How does AutoMapperLite compare with AutoMapper?" answer said "no compiled-expression engine," which is now actively wrong (AutoMapperLite has compiled each mapping into a cached delegate via `System.Linq.Expressions` since 4.4.0) and contradicted the FAQ's own structured-data (JSON-LD) answer to the same question on the same page. Updated to match the JSON-LD version ("no large convention engine").
  - `SECURITY.md`'s supported-versions table listed `4.2.x` as the supported line — `4.2.0` was never published to NuGet (see `CLAUDE.md`: `4.1.0`/`4.2.0`/`4.3.0` were internal-only development-session labels for stages of the work that shipped as `4.4.0`). Updated to reference `4.4.x`, the actual current published line.
  - `docs/performance.html`'s "Why" section overstated `Activator.CreateInstance`'s elimination, claiming it was "used only once, at compile time, never per mapped object" — untrue for the `ForPath` interpreted walk and the untyped collection-dispatch fallback (empty-collection / non-generic-`IEnumerable` source), both of which still call it at runtime. Reworded to scope the claim to what's actually eliminated (the compiled per-property path) and name the two remaining, deliberately-interpreted call sites.
- **CI:** removed the GitHub Actions publish workflow (`.github/workflows/publish.yml`); releases are packed and pushed to NuGet manually going forward.
- No public API breaking changes since 4.0.2.
